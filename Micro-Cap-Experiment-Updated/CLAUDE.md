...
- **Complete**: **52-week experiment closed at the 2026-09-11 close** — equity $733.51 vs S&P-equivalent $768.52, **gap −4.56%**, TWR alpha **+1.11%** (printed +0.55% until the TWR S&P-leg start-session fix). Now an **indefinite live process**: `experiment_config.json` has a null horizon + `benchmark_base_date: 2026-09-11`, so every metric is measured from that close (Sharpe/Sortino/CAPM N/A until 20 sessions). Cash floor back to 15%; ATRC partial cancelled; prohibited-business list enforced in screener + PRV gate (CXW withdrawn, OPFI blocklisted)
- **In progress**: **Screener redesign** — user leans to timing-based modes (option 3); an evidence-first plan (filter-before-rank, Finviz fundamentals, factor study on the 20 committed watchlists / 18 universe caches) is proposed. Found: `above_sma50` never computes (60-calendar-day lookback ≈ 40 sessions), "squeeze" is cross-sectional low volatility, volume ratio is one session. PGNY/HRMY: no catalyst inside 60 days → blocked by the RISK-OFF regime filter
